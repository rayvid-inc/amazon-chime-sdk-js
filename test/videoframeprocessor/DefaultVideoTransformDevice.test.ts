...
describe('DefaultVideoTransformDevice', () => {
  ...
  describe('onOutputStreamDisconnect', () => {
    ...
    it('stops the processing pipeline if it exists', async () => {
      await transformDevice.applyProcessors(mockVideoStream);
      // @ts-ignore
    ...
    it('does not release the input media stream if the provided device is media stream', async () => {
      ...
      await transformDevice.applyProcessors(streamAsDevice);
      transformDevice.onOutputStreamDisconnect();
    });
  });
  ...
  describe('observer callback', () => {
    ...
    it('processingDidFailToStart due to processor errors', done => {
      ...
      new TimeoutScheduler(300).start(() => {
        expect(obs.processingDidFailToStart.called).to.equal(true);
        device.stop();
      });
    ...
    it('processingLatencyTooHigh', done => {
      ...
      new TimeoutScheduler(800).start(() => {
        expect(obs.processingLatencyTooHigh.called).to.equal(true);
        device.stop();
        done();
      });
    });
